...
function stringifyComponentInfo(filepath, { from: path, as: name, name: importName }, importPathTransform) {
  if (!name)
    return void 0;
  path = _chunk44PFEKS6js.getTransformedPath.call(void 0, path, importPathTransform);
...
async function transformComponent(code, transformer2, s, ctx, sfcPath) {
  ...
  for (const { rawName, replace } of results) {
    debug2(`| ${rawName}`);
    const name = _chunk44PFEKS6js.pascalCase.call(void 0, rawName);
    ctx.updateUsageMap(sfcPath, [name]);
    const component = await ctx.findComponent(name, "component", [sfcPath]);
    if (component) {
      const varName = `__unplugin_components_${no}`;
      s.prepend(`${_chunk44PFEKS6js.stringifyComponentImport.call(void 0, _chunkBTQOTIPQjs.__spreadProps.call(void 0, _chunkBTQOTIPQjs.__spreadValues.call(void 0, {}, component), { as: varName }), ctx)};
`);
      no += 1;
      replace(varName);
    }
...
async function transformDirective(code, transformer2, s, ctx, sfcPath) {
  ...
  for (const { rawName, replace } of results) {
    debug3(`| ${rawName}`);
    const name = `${_chunk44PFEKS6js.DIRECTIVE_IMPORT_PREFIX}${_chunk44PFEKS6js.pascalCase.call(void 0, rawName)}`;
    ctx.updateUsageMap(sfcPath, [name]);
    const directive = await ctx.findComponent(name, "directive", [sfcPath]);
    if (!directive)
      continue;
    const varName = `__unplugin_directives_${no}`;
    s.prepend(`${_chunk44PFEKS6js.stringifyComponentImport.call(void 0, _chunkBTQOTIPQjs.__spreadProps.call(void 0, _chunkBTQOTIPQjs.__spreadValues.call(void 0, {}, directive), { as: varName }), ctx)};
`);
    no += 1;
    replace(varName);
  }
...
function transformer(ctx, transformer2) {
  return async (code, id, path) => {
    ...
    s.prepend(_chunk44PFEKS6js.DISABLE_COMMENT);
...
var Context = class {
  ...
  transform(code, id) {
    const { path, query } = _chunk44PFEKS6js.parseId.call(void 0, id);
  ...
  setupWatcher(watcher) {
    ...
    watcher.on("unlink", (path) => {
      if (!_chunk44PFEKS6js.matchGlobs.call(void 0, path, globs))
        return;
  ...
  onUpdate(path) {
    ...
    const name = _chunk44PFEKS6js.pascalCase.call(void 0, _chunk44PFEKS6js.getNameFromFilePath.call(void 0, path, this.options));
  ...
  async findComponent(name, type, excludePaths = []) {
    ...
    for (const resolver of this.options.resolvers) {
      if (resolver.type !== type)
        continue;
      const result = await resolver.resolve(type === "directive" ? name.slice(_chunk44PFEKS6js.DIRECTIVE_IMPORT_PREFIX.length) : name);
      if (!result)
        continue;
      if (typeof result === "string") {
        ...
        info = _chunkBTQOTIPQjs.__spreadValues.call(void 0, {
          as: name
        }, _chunk44PFEKS6js.normalizeComponetInfo.call(void 0, result));
      }
      if (type === "component")
  ...
  normalizePath(path) {
    var _a, _b, _c;
    return _chunk44PFEKS6js.resolveAlias.call(void 0, path, ((_b = (_a = this.viteConfig) == null ? void 0 : _a.resolve) == null ? void 0 : _b.alias) || ((_c = this.viteConfig) == null ? void 0 : _c.alias) || []);
  }
  relative(path) {
...
var unplugin_default = _unplugin.createUnplugin.call(void 0, (options = {}) => {
  ...
  const api = {
    ...
    stringifyImport(info) {
      return _chunk44PFEKS6js.stringifyComponentImport.call(void 0, info, ctx);
    }
  };
  return {
    ...
    async transform(code, id) {
      if (!_chunk44PFEKS6js.shouldTransform.call(void 0, code))
        return null;
      try {
